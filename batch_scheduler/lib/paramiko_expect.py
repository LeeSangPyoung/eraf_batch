...
def strip_ansi_codes(s):
    return re.sub(
        r"\x1b\[([0-9,A-Z]{1,2}(;[0-9]{1,2})?(;[0-9]{3})?)?[m|K]?|\?(1049|2004)[hl]",
        "",
        s,
    )
...
class SSHClientInteraction(object):
    ...
    def __init__(
        ...
        display=False,
        encoding="utf-8",
        output_callback=default_output_func,
        tty_width=80,
        tty_height=24,
        lines_to_check=1,
        term="vt100",
        ...
        self.channel = client.invoke_shell(
            term=term, width=tty_width, height=tty_height
        )
    ...
    def expect(
        ...
        while len(re_strings) == 0 or not [
            re_string
            for re_string in re_strings
            if re.match(
                default_match_prefix + re_string + "$",
                current_buffer_output_decoded,
                re.DOTALL,
            )
        ]:
            ...
            current_buffer_output_decoded = "\n" + "\n".join(
                self.current_output.splitlines()[-lines_to_check:]
            )
        ...
        if len(self.current_send_string) != 0:
            self.current_output_clean = self.current_output_clean.replace(
        ...
        if len(re_strings) != 0 and len(found_pattern) != 0:
            self.current_output_clean = re.sub(
                            ...
                            output_callback(
                                {
                                    ...
                                    "current_line_decoded": current_line_decoded,
                                }
                            )
                        else:
                            output_callback(
                                {"id": id, "current_line_decoded": current_line_decoded}
                            )
    ...
    def take_control(self):
        ...
        if has_termios:
            ...
            try:
                ...
                while True:
                    select_read, select_write, select_exception = select.select(
