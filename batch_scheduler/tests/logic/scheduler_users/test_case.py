...
def test_scheduler_users_read(monkeypatch):
    ...
    result = scheduler_users.scheduler_users_read(None, {})

    assert result == {
        "data": mock_data.mock_result,
        "total": len(mock_data.mock_result),
    }
...
def test_scheduler_users_create(monkeypatch):
    ...
    result = scheduler_users.scheduler_users_create({})
...
def test_scheduler_users_update(monkeypatch):
    ...
    result = scheduler_users.scheduler_users_update({"id": "test"})
