...
class SshClient:
    ...
    def create_ssh_client(self, host, username=None, password=None):
        try:
            host, username = self.parse_ssh_user(host, username)
    ...
    @staticmethod
    def parse_ssh_user(host_ip_addr, ssh_user):
        if "@" in host_ip_addr:
            ssh_user, host_ip_addr = host_ip_addr.split("@")

        return host_ip_addr, ssh_user
    ...
    def ssh_execute_command(self, ssh, command):
        ...
        try:
            ...
            # Read stderr if there's any error output
            stderr_output = stderr.read().decode("utf-8").strip()
            if stderr_output:
                self.logger.error(f"STDERR: {stderr_output}")
                error = stderr_output

            # If command failed but no stderr, set a generic error
    ...
    def copy_celery_worker_code(self, host_ip_addr, ssh_user, params):
        ...
        try:
            ...
            success, error = self.ssh_execute_command(
                ssh_client, f"(set -e; {'; '.join(commands)})"
            )
    ...
    def copy_celery_worker_code_v2(self, host_ip_addr, ssh_user, folder_path):
        self.logger.info("============ Copy celery worker code")
        self.logger.info(
            f"============ Copy celery worker code to {ssh_user}@{host_ip_addr}/{folder_path}"
        )
        ssh_client = self.create_ssh_client(host_ip_addr, ssh_user)
        try:
    ...
    def clean_old_worker_folder(self, check_record):
        ...
        self.logger.debug(f"clean_old_worker_folder check_record: {check_record}")
        ssh_client = self.create_ssh_client(
            check_record.get("host_ip_addr"), check_record.get("ssh_user")
        )
        try:
            commands = [f"rm -rf {check_record.get('folder_path')}/celery"]
            self.logger.debug(f"commands: {commands}")
            self.ssh_execute_command(ssh_client, f"(set -e; {'; '.join(commands)})")
            ssh_client.close()
    ...
    def start_celery_worker(self, host_ip_addr, ssh_user, params):
        ...
        try:
            commands = [
                ...
                "dos2unix .env",
                f"grep -q '^QUEUE_NAME=' .env || echo \"QUEUE_NAME={params['queue_name']}\" >> .env",
                "./start_worker.sh",
            ]
            success, error = self.ssh_execute_command(
                ssh_client, f"(set -e; {'; '.join(commands)})"
    ...
    def start_celery_worker_v2(self, host_ip_addr, ssh_user, folder_path, queue_name):
        ...
        try:
            commands = [
                ...
                "dos2unix start_worker.sh",
                "dos2unix .env",
                f"grep -q '^QUEUE_NAME=' .env || echo \"QUEUE_NAME={queue_name}\" >> .env",
                "./start_worker.sh",
            ]
            success, error = self.ssh_execute_command(
                ssh_client, f"(set -e; {'; '.join(commands)})"
    ...
    def restart_celery_worker_v2(self, host_ip_addr, ssh_user, folder_path):
        self.logger.info("============ ReStart celery worker")
        ssh_client = self.create_ssh_client(host_ip_addr, ssh_user)
        try:
            commands = [
                f"cd {folder_path}/celery",
    ...
    def read_log_server(self, params, callback):
        ...
        line_prefix = f" {job_id}:" if job_id else "worker_id_not_found :"
