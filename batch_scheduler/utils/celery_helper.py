...
class CeleryClient:
    ...
    def celery_delete_jobs(self, params):
        self.logger.info("============ celery_delete_jobs")
        payload = self.__build_celery_payload(params)
        self.__process_job_data(params, payload)
        return self.fetch_data("DELETE", "/celerytasks/delete_task/", payload)
    ...
    def celery_manually_run(self, params):
        ...
        return self.fetch_data("POST", "/celerytasks/manually_run/", params, [500])

    def celery_force_stop(self, params):
        self.logger.info("============ celery_force_stop")
        return self.fetch_data(
            "POST", "/celerytasks/force_stop/", params, white_list_status=[404]
        )

    def celery_delete_workflow(self, params):
        self.logger.info("============ celery_delete_workflow")
        return self.fetch_data("POST", "/celerytasks/delete_workflow/", params)
    ...
    def fetch_data(self, method, path, payload, white_list_status: list = None):
        white_list_status = white_list_status or []
        ...
        if (
            response.status == 200
            and json.loads(response.data).get("status") == "success"
        ) or response.status in white_list_status:
