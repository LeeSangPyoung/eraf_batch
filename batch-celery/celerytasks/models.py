...
class JobSettings(models.Model):
    ...
    workflow_delay = models.IntegerField(default=0, blank=True, null=True)
    ...
    job_type = models.CharField(
        max_length=50, choices=JobTypeEnum.choices, default=JobTypeEnum.EXECUTABLE
    )
    ...
    last_result = models.CharField(
        max_length=255, choices=STATUS_CHOICES, default=STATUS_CREATED
    )
    ...
    def save_operation(self, operation, task_status):
        self.job_operation = operation
        status = (
            JobSettings.STATUS_SUCCESS
            if task_status == TaskDetail.STATUS_SUCCESS
            else JobSettings.STATUS_FAILURE
        )
    ...
    def update_workflow(
        self, workflow_id, priority, ignore_result, workflow_delay: int = 0
    ):
        """
        Update the workflow for the job.
        """
        self.workflow_id = workflow_id
        self.priority = priority
        self.ignore_result = ignore_result
        self.workflow_delay = workflow_delay
        self.save(
            update_fields=(
                "workflow_id",
                "priority",
...
class TaskDetail(models.Model):
    ...
    def update_last_result(self, result):
        ...
        self.save(update_fields=("task_result",))
        return self
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["job", "status"], name="idx_task_job_status"),
        ]


class TaskWorkflow(models.Model):
    """
    Workflow is a collection of tasks that are scheduled to run in a group.
    """

    workflow_id = models.CharField(max_length=255, primary_key=True)
    repeat_interval = models.TextField(max_length=4000, blank=False, null=False)
    timezone = models.CharField(max_length=255, blank=True, null=True)
    queue_name = models.CharField(max_length=255, blank=True, null=True)
    start_date = models.DateTimeField(blank=True, null=True)
    last_run_date = models.DateTimeField(blank=True, null=True)
    ...
    def update(
        ...
        """
        Update the task workflow with the given parameters.
        """
