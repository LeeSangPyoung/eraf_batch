...
const useFilterData = (onSystemSuccess = null, onGroupSuccess = null, onWorkflowSuccess = null) => {
  ...
  const { data: jobData, mutate: jobFilterMutation, error: jobFilterError } = useSWR(['/job/getFilter', group, jobOffset, jobSearchTextInput, setJobs], async ([url, group, jobOffset, jobSearchTextInput, setJobs]) => {
    try {
      if (isWorkflow && !group) {
        return;
      }
