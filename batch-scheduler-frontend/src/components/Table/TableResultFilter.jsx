...
import { operationOptions } from '../../utils/enum';
...
function TableResultFilter({
  ...
  return (
    <div className="grid lg:grid-cols-4 md:grid-cols-3 gap-3 w-full">
      {visibleJobs &&
        serverFilter &&
        visibleGroups &&
        job &&
      ...
      <SearchAutocomplete
        ...
        value={inputCombo.operation}
        inputValue={inputCombo.operationInput}
        onChange={(event, newValue) =>
          handleValueChange(event, newValue, 'operation')
        }
        onInputChange={(event, newValue) =>
          handleInputChange(event, newValue, 'operation')
        }
        content={t('operation')}
      />
      <SearchAutocomplete
...
export default TableResultFilter;
